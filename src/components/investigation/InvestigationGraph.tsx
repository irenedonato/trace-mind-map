      ...
      {!isRunning && visibleNodes.length === 0 && (
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm">Configure seed data and launch investigation</p>
            <p className="text-muted-foreground/50 text-xs mt-1">Graph will expand as agents discover relationships</p>
          </div>
        </div>
      )}
      ...
      <svg className="absolute inset-0 w-full h-full" style={{ pointerEvents: "none" }}>
        <defs>
          <marker id="arrowhead" markerWidth="8" markerHeight="6" refX="8" refY="3" orient="auto">
            <polygon points="0 0, 8 3, 0 6" fill="hsl(215, 10%, 57%)" opacity="0.5" />
          </marker>
            ...
            return (
              <motion.g key={edge.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6 }}>
                <line
                  ...
                  x2={target.x}
                  y2={target.y}
                  stroke={inPath ? "hsl(217, 91%, 60%)" : edge.inferred ? "hsl(38, 92%, 50%)" : "hsl(215, 14%, 25%)"}
                ...
                />
                <rect x={midX - 30} y={midY - 10} width="60" height="20" rx="3" fill="hsl(215, 20%, 9%)" opacity="0.9" />
                <text x={midX} y={midY + 4} textAnchor="middle" fill="hsl(215, 10%, 57%)" fontSize="9" fontFamily="'JetBrains Mono', monospace">
                  {edge.label}
                </text>
              </motion.g>
            );
